class Ingredient
  ...
  def valid_ingredient?
    ...
    if acceptable_ingredients.include?(@name.downcase)
      true
    else
      false
    end
  ...
  def self.parse(unparsed_ingredient)
    split_ingredient = unparsed_ingredient.split
    quantity = split_ingredient.shift
...
class Recipe
  ...
  def can_haz?
    ...
    @ingredients.each do |ingredient|
      validations << ingredient.valid_ingredient?
    end

    if validations.include?(false)
      "No can haz!"
    else
      "Can haz!"
    end
...
recipes = [
  ...
  Recipe.new("Quinoa Whiskey Milk", [
    Ingredient.new(0.5, "tablespoon", "brussels sprouts"),
    Ingredient.new(1.5, "tablespoons", "eggs"),
    Ingredient.new(0.33, "cup", "spinach"),
    Ingredient.new(0.25, "gallon", "milk"),
    Ingredient.new(1, "teaspoon", "tofu"),
    Ingredient.new(0.5, "tablespoon", "seitan"),
    Ingredient.new(0.5, "cup", "bell peppers"),
    Ingredient.new(0.25, "cup", "quinoa"),
    Ingredient.new(0.25, "cup", "kale"),
    Ingredient.new(2.5, "tablespoons", "chocolate"),
    Ingredient.new(0.5, "teaspoon", "beer"),
    Ingredient.new(0.25, "teaspoon", "wine"),
    Ingredient.new(0.25, "liter", "whiskey"),
  ], [
      "In a small bowl or mug, mix the whiskey and milk. Set aside.",
      "In a medium sized bowl stir together the eggs, tofu, and spinach until incorporated. Stir until combined.",
      "In another bowl, whisk the dry ingredients together. Add the wet mixture to the dry mixture and stir well.",
      "Mix the kale and beer together in a small bowl.",
      "Bake for 10-11 minutes.",
    ]),
]

recipes.each do |recipe|
  puts "Can client haz \"#{recipe.name}\"? #{recipe.can_haz?}"
end
